...
def init_db():
    ...
    for col, ddl in [
        ...
        ("theme_bg", "ALTER TABLE users ADD COLUMN theme_bg TEXT DEFAULT ''"),
    ]:
    ...
    if "text_content" not in existing_story_cols:
        conn.execute("ALTER TABLE stories ADD COLUMN text_content TEXT DEFAULT ''")
...
def upload_story():
    ...
    conn = get_db()
    # A person can post several statuses - each one shows for 24 hours,
    # like WhatsApp Status (they stack, they don't replace each other).
...
@app.route("/story/upload-text", methods=["POST"])
@login_required
def upload_text_story():
    ...
    now = datetime.utcnow()
    conn = get_db()
    conn.execute("""INSERT INTO stories (id, user_id, media_url, type, created_at, expires_at, text_content)
                    VALUES (?, ?, ?, ?, ?, ?, ?)""",
                 (story_id, user['id'], bg_color, "text",
                  now.isoformat(), (now + timedelta(hours=24)).isoformat(), text))
    conn.commit()
    conn.close()
    return redirect(url_for('dashboard'))
...
def profile():
    ...
    if request.method == "POST":
        ...
        theme_bg = request.files.get("theme_bg")
        if theme_bg and theme_bg.filename:
            ext = os.path.splitext(secure_filename(theme_bg.filename))[1]
            fname = f"bg_{user['id']}_{uuid.uuid4().hex}{ext}"
            theme_bg.save(os.path.join(PROFILE_DIR, fname))
            conn.execute("UPDATE users SET theme_bg = ? WHERE id = ?", (fname, user['id']))
        if request.form.get("remove_theme_bg") == "1":
            conn.execute("UPDATE users SET theme_bg = '' WHERE id = ?", (user['id'],))
